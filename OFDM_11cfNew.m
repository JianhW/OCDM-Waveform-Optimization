function C_opt = OFDM_11cfNew(c, C_idx, R_idx, varargin)
% OPTIMIZE_WAVEFORM_OFDM_V2  基于对比文献2的OFDM波形优化函数
%
% 功能：
...
%
% ─── 调用格式 ───────────────────────────────────────────────────────────
%   C_opt = optimize_waveform_ofdm_v2(c, C_idx, R_idx)
%   C_opt = optimize_waveform_ofdm_v2(c, C_idx, R_idx, Name, Value, ...)
%
% ─── 必选输入 ────────────────────────────────────────────────────────────
%   c       - 输入频域波形，(N×1) 复数列向量
...
%
% ─── 输出 ─────────────────────────────────────────────────────────────────
%   C_opt   - 优化后的频域波形，(N×1) 复数列向量
%
% ─── 算法流程（基于对比文献2 cpt11_10_1.m）─────────────────────────────────
%   Step 0: x = ifft([c; 0…0], LN) * √L          (补零过采样 IFFT)
%   For it = 1 : Iter
%     Step 1: T_clip = CR · ‖x‖/√LN               (削峰阈值)
%             x_hat = clip(x, T_clip)              (幅度限幅)
%     Step 2: c_in = fft(x_hat)[1:N] / √L         (提取带内分量)
...
%     Step 5: C_new = [H ⊙ c_in; 0…0]              (新频域信号)
%             x = ifft(C_new) * √L                  (IFFT回时域)
%     Step 6: x = clip(x, T_peak)                  (二次轻削峰)
%   End
%   Step 7: C_opt = fft(x)(1:N) / √L               (提取带内频域输出)
%
% ─── 示例 ─────────────────────────────────────────────────────────────────
%   N = 128;  L = 4;
%   % 生成 QPSK 频域码元
...
%
%   % 场景1：所有子载波用于通信，无雷达子载波
%   C_opt = optimize_waveform_ofdm_v2(c, 1:N, []);
%
%   % 场景2：ISAC 场景，前100个通信，后28个雷达
%   C_idx = 1:100;   R_idx = 101:128;
%   C_opt = optimize_waveform_ofdm_v2(c, C_idx, R_idx, 'L', 4, 'Iter', 2);
%
% 版本：v1.0  (2026-04-06)
...
%% ──────────────── 参数检验与解析 ────────────────────────────────────────
if nargin < 3
    error('optimize_waveform_ofdm_v2: 至少需要 3 个输入参数 (c, C_idx, R_idx)');
end
...
if ~isempty(C_idx) && ~isempty(R_idx) && ~isempty(intersect(C_idx, R_idx))
...
%% ──────────────── 预生成 IFFT 变换矩阵 A ────────────────────────────────
% A(i,k) = (√L/LN) * exp(j·2π·i·k / LN)，(i,k 均从 0 起)
% 等价于：x ≈ A * c_in  →  ifft([c_in; zeros(…)]) * √L
% 与对比文献2 cpt11_10_1.m 中公式一致
if isempty(A)
    n_vec = (0:LN-1).';
    k_vec = 0:(N-1);
    A = (sqrt(L)/LN) * exp(1j * 2*pi * (n_vec * k_vec) / LN);  % LN×N
end
...
if use_cvx
    ...
    if exist('cvx_begin','file') == 2
        ...
        if solver_ok
            ...
            if verbose
                warning('optimize_waveform_ofdm_v2: CVX 求解器均不可用，将使用近似求解');
            end
        end
    else
        if verbose
            warning('optimize_waveform_ofdm_v2: 未检测到 CVX，将使用近似求解');
        end
    end
end

%% ──────────────── 时域初始化 ────────────────────────────────────────────
% 与对比文献2一致：x = ifft([c; zeros((L-1)*N,1)], LN) * sqrt(L)
c_padded = [c; zeros((L-1)*N, 1)];
x = ifft(c_padded, LN) * sqrt(L);
...
for it = 1:iters
    ...
    %--- Step 1: 削峰 ---
    % 与对比文献2一致：T = CR * (norm(x)/sqrt(LN))
    T_clip = CR * norm(x) / sqrt(LN);
    x_hat  = local_clip(x, T_clip);

    %--- Step 2: 提取带内分量 ---
    % 与对比文献2一致：C_hat = sqrt(1/L) * fft(x_hat, LN)
    C_hat = sqrt(1/L) * fft(x_hat, LN);
    c_in  = C_hat(1:N);              % N×1，带内分量
    ...
    if cvx_available
        try
            ...
            if strcmpi(cvx_status, 'Solved')
                ...
                H_opt = local_feas_project(ones(N,1), c_in, L, N, T_peak);
            end
        ...
        %--- 无 CVX：使用近似求解（对比文献2中的 fallback 策略）---
        stat.approx = stat.approx + 1;
        H_opt = local_feas_project(ones(N,1), c_in, L, N, T_peak);
    end

    %--- Step 5: 重建时域波形 ---
    % 与对比文献2一致：C_new = zeros(LN,1); C_new(1:N) = H_opt.*c_in
    C_new        = zeros(LN, 1);
    ...
    x = ifft(C_new, LN) * sqrt(L);

    %--- Step 6: 二次轻削峰 ---
    % 与对比文献2一致：a = abs(x); idx = a > T_peak; if any(idx), x(idx) = T_peak * x(idx) ./ a(idx); end
    x = local_clip(x, T_peak);
end

%% ──────────────── 输出频域波形 ──────────────────────────────────────────
% 与对比文献2一致：提取带内频域分量
C_full = sqrt(1/L) * fft(x, LN);   % 完整过采样频域
C_opt  = C_full(1:N);              % 提取带内 N 点

if verbose
    papr_db = 10 * log10(max(abs(x).^2) / mean(abs(x).^2));
    fprintf('[optimize_waveform_ofdm_v2] 完成 | PAPR=%.2f dB | ', papr_db);
...
function H = local_feas_project(H_init, c_in, L, N, T_peak)
%LOCAL_FEAS_PROJECT  可行化投影（无 CVX 时的回退方案）
%   与对比文献2 feas_project 函数一致
%   一次频域→时域→削峰→频域投影
    LN = L * N;
    spec = zeros(LN, 1);
    spec(1:N) = H_init .* c_in;
    xt = ifft(spec, LN) * sqrt(L);          % 时域
    xt = local_clip(xt, T_peak);            % 削峰
    Cxt = sqrt(1/L) * fft(xt, LN);          % 回频域
