function z_opt = Varshney(z, idx_comm, idx_radar, varargin)
...
%             默认恢复输入 z 的频域幅度，仅使用 Varshney 算法更新雷达相位；
%             preserveMagnitude=false 时输出源算法的所有子载波单位模结果。
%
% ─────────────────────────────────────────────────────────────────────────────
% 使用示例
%
...
addParameter(p, 'preserveMagnitude', true, @islogical);
...
preserveMagnitude = p.Results.preserveMagnitude;
...
if preserveMagnitude
    z_phase = ones(N, 1);
    phase_mask = abs(z) >= tol_mag;
    z_phase(phase_mask) = z(phase_mask) ./ abs(z(phase_mask));
    z_opt = input_mag .* z_phase;
end
